...
describe('SchedulerBrowser', () => {
  ...
  function installMockBrowserRuntime() {
    ...
    const scheduling = {
      isInputPending(options) {
        if (this !== scheduling) {
          throw new Error('isInputPending called with incorrect `this` context');
        }
        return hasPendingDiscreteEvent || options && options.includeContinuous && hasPendingContinuousEvent;
      }
    };
    global.navigator = {
      scheduling
  ...
  it('when isInputPending is available, we can wait longer before yielding', () => {
    ...
    runtime.assertLog(['Post Message']);
    runtime.fireMessageEvent();
    runtime.assertLog(['Message Event', 'Task with no pending input',
    ...
    gate(flags => flags.enableIsInputPending ? 'Yield at 10ms' :
    ...
    runtime.assertLog(['Post Message']);
    runtime.fireMessageEvent();
    runtime.assertLog(['Message Event', 'Task with pending input',
    // This time we yielded quickly to unblock the discrete event.
    'Yield at 5ms', 'Discrete Event']);
  });
  it('isInputPending will also check for continuous inputs, but after a ' + 'slightly larger threshold', () => {
    function blockUntilSchedulerAsksToYield() {
      while (!Scheduler.unstable_shouldYield()) {
        runtime.advanceTime(1);
      }
  ...
  it('requestPaint forces a yield at the end of the next frame interval', () => {
    ...
    runtime.assertLog(['Post Message']);
    runtime.fireMessageEvent();
    runtime.assertLog(['Message Event', 'Task with no paint', gate(flags => flags.enableIsInputPending ? 'Yield at 10ms' :
    // When isInputPending is disabled, we always yield quickly
    'Yield at 5ms')]);
    runtime.resetTime();
    ...
    runtime.assertLog(['Post Message']);
    runtime.fireMessageEvent();
    runtime.assertLog(['Message Event', 'Task with paint',
  ...
  it('yielding continues in a new task regardless of how much time is remaining', () => {
    ...
    runtime.assertLog(['Post Message']);
    runtime.fireMessageEvent();
    runtime.assertLog(['Message Event', 'Original Task',
