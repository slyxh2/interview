...
describe('Scheduler', () => {
  const {
    enableProfiling
  } = require('scheduler/src/SchedulerFeatureFlags');
  if (!enableProfiling) {
  ...
  function stopProfilingAndPrintFlamegraph() {
    ...
    const tasksByPriority = Array.from(tasks.values()).sort((t1, t2) => t1.priorityLevel - t2.priorityLevel);
    for (const task of tasksByPriority) {
      let label = task.label;
      if (label === undefined) {
        label = 'Task';
      }
      let labelColumn = `Task ${task.id} [${priorityLevelToString(task.priorityLevel)}]`;
      ...
      for (const time of task.runs) {
        const index = time / microsecondsPerChar;
        timelineColumn += (isRunning ? '█' : '░').repeat(index - timelineColumn.length);
        isRunning = !isRunning;
      }
...
`);
  ...
  it('automatically stops profiling and warns if event log gets too big', async () => {
    ...
    expect(console.error.mock.calls[0][0]).toBe("Scheduler Profiling: Event log exceeded maximum size. Don't forget " + 'to call `stopLoggingProfilingEvents()`.');
