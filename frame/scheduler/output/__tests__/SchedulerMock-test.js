...
describe('Scheduler', () => {
  ...
  it('cancels work', async () => {
    scheduleCallback(NormalPriority, () => Scheduler.log('A'));
    const callbackHandleB = scheduleCallback(NormalPriority, () => Scheduler.log('B'));
    scheduleCallback(NormalPriority, () => Scheduler.log('C'));
  ...
  it('continuations do not block higher priority work scheduled ' + 'inside an executing callback', async () => {
    const tasks = [['A', 100], ['B', 100], ['C', 100], ['D', 100]];
    const work = () => {
      while (tasks.length > 0) {
    ...
    scheduleCallback(NormalPriority, work);
    await waitForAll(['A', 'B', 'Schedule high pri',
  ...
  it('wrapped callbacks have same signature as original callback', () => {
    const wrappedCallback = wrapCallback((...args) => ({
      args
    }));
  ...
  it('wrapped callbacks inherit the current priority', () => {
    const wrappedCallback = runWithPriority(NormalPriority, () => wrapCallback(() => {
      Scheduler.log(getCurrentPriorityLevel());
    }));
    const wrappedUserBlockingCallback = runWithPriority(UserBlockingPriority, () => wrapCallback(() => {
      Scheduler.log(getCurrentPriorityLevel());
    }));
    wrappedCallback();
  ...
  it('wrapped callbacks inherit the current priority even when nested', () => {
    ...
    runWithPriority(NormalPriority, () => {
      ...
      wrappedUserBlockingCallback = runWithPriority(UserBlockingPriority, () => wrapCallback(() => {
        Scheduler.log(getCurrentPriorityLevel());
      }));
    });
    wrappedCallback();
  ...
  if (__DEV__) {
    ...
    it.skip('adds extra function to the JS stack whose name includes the priority level', async () => {
      function inferPriorityFromCallstack() {
        try {
          ...
          for (let i = lines.length - 1; i >= 0; i--) {
            const line = lines[i];
            const found = line.match(/scheduler_flushTaskAtPriority_([A-Za-z]+)/);
            if (found !== null) {
      ...
      scheduleCallback(ImmediatePriority, () => Scheduler.log('Immediate: ' + inferPriorityFromCallstack()));
      scheduleCallback(UserBlockingPriority, () => Scheduler.log('UserBlocking: ' + inferPriorityFromCallstack()));
      scheduleCallback(NormalPriority, () => Scheduler.log('Normal: ' + inferPriorityFromCallstack()));
      scheduleCallback(LowPriority, () => Scheduler.log('Low: ' + inferPriorityFromCallstack()));
      scheduleCallback(IdlePriority, () => Scheduler.log('Idle: ' + inferPriorityFromCallstack()));
  ...
  describe('delayed tasks', () => {
    ...
    it('cancels a delayed task', async () => {
      ...
      const taskB = scheduleCallback(NormalPriority, () => Scheduler.log('B'), options);
      const taskC = scheduleCallback(NormalPriority, () => Scheduler.log('C'), options);
    ...
    it('toFlushUntilNextPaint stops if a continuation is returned', async () => {
      ...
      await waitForPaint(['Original Task',
    ...
    it("toFlushAndYield keeps flushing even if there's a continuation", async () => {
      ...
      await waitForAll(['Original Task',
      // Immediately before returning a continuation, `shouldYield` returns
      // false, which means there must be time remaining in the frame.
      'shouldYield: false', 'Return a continuation',
